...
int protected_generic_interface(SSL_CTX *upstream_context, struct sockaddr *downstream_sockaddr, struct sockaddr *upstream_sockaddr, char *hostname, bool protected_downstream, int sockfd, int timeout) {
    ...
    /* Ensure our parameters are valid. */
    if (!downstream_sockaddr || !upstream_sockaddr || sockfd < 0 || !hostname || !upstream_context) {
    ...
    while (1) {
        ...
        /* Create our S.S.L. structure. */
        SSL *ssl = SSL_new(upstream_context);
        SSL_set_fd(ssl, client_sockfd);
        if (SSL_accept(ssl) <= 0) {

            fprintf(stderr, "There was an error while trying to negotiate a T.L.S. handshake.\n");
            SSL_free(ssl);
            close(client_sockfd);
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    if (events[i].events & EPOLLIN) {
                        ...
                        if (fd == downstream_to_child_fd[0]) {
                            ...
                            /* Write the downstream message to the upstream client. */
                            if (SSL_write(ssl, downstream_message, downstream_message_length) < 0) {
                        ...
                        /* Something has been written to us from the upstream client. */
                            ...
                            size_t upstream_message_length = 8000000;
                            char *upstream_message = (char *) malloc(upstream_message_length);
                            if ((upstream_message_length = SSL_read_ex(ssl, upstream_message, upstream_message_length, &upstream_message_length)) <= 0) {
            ...
            while (1) {
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    if (events[i].events & EPOLLIN) {
                        ...
                        /* Something has been written to us from the upstream client. */
                        if (fd == child_to_downstream_fd[0]) {
                            fprintf(stderr, "written to us by upstream client\n");
                        ...
                        /* Something has been written to us from the downstream server. */
                            ...
                            fprintf(stderr, "written to us by downstream client\n");
